...
class GTRiskPointSampler:
    ...
    def __call__(self, depth: torch.Tensor, pos_w: torch.Tensor, rot_wb: torch.Tensor, map_id: int):
        ...
        y = points_b[:, 1]
        z = points_b[:, 2]
        forward = (x > self.min_forward_m) & (x < min(self.max_forward_m, self.depth_max_m))
        yaw = torch.atan2(y, x.clamp(min=1e-4))
        planar = torch.sqrt(x.square() + y.square()).clamp(min=1e-4)
        pitch = torch.atan2(z, planar)
        in_fov = (yaw.abs() < 0.5 * self.horizon_fov) & (pitch.abs() < 0.5 * self.vertical_fov)
        candidate_mask = forward & in_fov
        if not bool(candidate_mask.any()):
            return empty_points, empty_weight

        height, width = depth.shape[-2:]
        u = ((yaw[candidate_mask] / self.horizon_fov) + 0.5) * width - 0.5
        v = (0.5 - pitch[candidate_mask] / self.vertical_fov) * height - 0.5
        ui = u.round().long().clamp(0, width - 1)
        vi = v.round().long().clamp(0, height - 1)
        observed_depth = depth[0, vi, ui].to(dtype) * self.depth_max_m
        candidate_depth = x[candidate_mask]
        hidden = candidate_depth > (observed_depth + self.hidden_depth_margin_m)
        if not bool(hidden.any()):
            return empty_points, empty_weight

        hidden_points_w = points_w[candidate_mask][hidden]
        hidden_depth = candidate_depth[hidden]
        observed = observed_depth[hidden]
        hidden_gap = (hidden_depth - observed).clamp(min=0.0)
        score = hidden_gap / hidden_gap.max().clamp(min=1e-3)
